...
class EarlyStoppingHandler:
    def __init__(self, early_stopping_thresholds: Optional[Dict[str, float]] = None):
...
class SAPEmbeddingsInitializer:
    def ensure_sap_embeddings_initialized(self, config: Dict[str, Any], trial_dir: str):
    ...
    def save_intermediate_result(self, component: str, working_df: pd.DataFrame, 
        ...
        try:
            ...
            summary = {
                ...
                'score': self.get_component_score(component, results),
            ...
            if config:
                summary['config_explored'] = {
                    'full_config': config,
                    'component_specific_config': self.extract_component_config(component, config)
                }
            ...
            if component in ['retrieval', 'query_expansion']:
                ...
                summary['compressor_metrics'] = results.get('compressor_metrics', {})
